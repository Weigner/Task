...
class TaskRepository(val context: Context) : BaseRepository(context) {
    ...
    fun all(listener: APIListener<List<TaskModel>>) {
        val call: Call<List<TaskModel>> = mRemote.findAll()
        list(call, listener)
    }

    fun nexWeek(listener: APIListener<List<TaskModel>>) {
        val call: Call<List<TaskModel>> = mRemote.findNextWeek()
        list(call, listener)
    }
    ...
    fun findById(id: Int, listener: APIListener<TaskModel>) {
        ...
        val call: Call<TaskModel> = mRemote.findById(id)
